...
def test_constructor():
    ...
    IA, IB, IC = qt.qeye(flux_A.nlev), qt.qeye(
        flux_B.nlev), qt.qeye(transmon.nlev)
...
def test_get_raised_op():
    nlev = 50
    flux_A: Subsystem = opt.build_optimized_system(
        flux_constr, flux_A_params, nlev=nlev)
    flux_B: Subsystem = opt.build_optimized_system(
        flux_constr, flux_B_params, nlev=nlev)
    transmon: Subsystem = opt.build_optimized_system(
        transmon_constr, transmon_params, nlev=nlev)
    subsys_dict: dict[str, Subsystem] = {'A': flux_A,
                                         'B': flux_B,
                                         'C': transmon}
    subsys_idxs: dict[str, Subsystem] = {'A': 0,
