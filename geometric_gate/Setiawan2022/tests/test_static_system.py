...
with open(yaml_path, 'r') as stream:
    try:
        ct_params: dict[str, dict] = yaml.safe_load(stream)['pulse_gen_ct']
...
def test_build_bare_system():
    ...
    stable_levels: int = 5
    flux_A: Subsystem = opt.build_optimized_system(
        flux_constr, flux_A_params, stable_levels)
    flux_B: Subsystem = opt.build_optimized_system(
        flux_constr, flux_B_params, stable_levels)
    transmon: Subsystem = opt.build_optimized_system(
        transmon_constr, transmon_params, stable_levels)
...
def test_build_interaction_H():
    ...
    assert not is_diag(
        interaction_H), 'Interaction Hamiltonian must not be diagonal'
...
def test_build_static_system():
    ...
    assert not is_diag(
        module_static_sys.H), 'Static System Hamiltonian should not be diagonal'
...
def plot_eigenen_varying_params(kwargs: dict[str, tuple[list[int], dict[str, Any]]],
    ...
    for key in kwargs:
        ...
        for i, value in enumerate(vary_params):
            ...
            # make sure zero of energy is equal to energy of the ground state for all models
            eigenen -= eigenen[0]
            eigenen_list[i, :] = eigenen
            for n, subsys_lbl in enumerate(subsys_lbls):
                subsys: Qobj = sys.subsystems[subsys_lbl]
                subsys_eig: np.ndarray[float] = subsys.H.eigenenergies()[
                    :max_level]
        ...
        for n, lbl in enumerate(subsys_lbls):
            ...
            for i in range(subsys_eigenen_list.shape[2]):
                ...
                ax_subs[n].plot(vary_params, eigen_over_n,
                                label=f'energy level {i}')
        ax.legend()
...
def run_plots(stable_nlev: list[int], trunc: list[int], nlev: list[int], max_level: int = 5) -> None:
    ...
    plot_eigenen_varying_params(
        kwargs, static_system.build_bare_system, 'bare_system')
    print('Making plots for dressed static system')
    plot_eigenen_varying_params(
        kwargs, static_system.build_static_system, 'static_system')
    print('Done.')
