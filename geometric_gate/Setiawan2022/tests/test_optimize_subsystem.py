...
with open(yaml_path, 'r') as stream:
    try:
        ct_params: dict = yaml.safe_load(stream)['pulse_gen_ct']
...
def test_stabilize_nlev():
    ...
    transmon_constr: Callable['...',
                              Qsys] = subsystems.build_transmon_operators
...
def test_diagonalize_Qsys():
    ...
    for lbl in transmon:
        assert transmon[lbl] == transmon_diag[lbl],\
            'transmon operator {} not left invariant by diagonlization'.format(
                lbl)
...
def test_build_optimized_system():
    ...
    for stable_nlev in stable_nlist:
        ...
        for truncate_to in trunc_nlist:
            ...
            assert opt.is_diag(
                transmon['H']), 'transmon hamiltonian not diagnoal'
