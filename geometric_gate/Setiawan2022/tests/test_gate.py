...
ct_params: dict[str, float | dict[str, float]] =\
    file_io.get_params('./config/circuit_parameters.yaml')
circuit: CompositeSystem = build_static_system(ct_params)
n_comp_states = 3
...
def test_assemble_empty_profile():
    ...
    gate_profile: GateProfile = gate.assemble_empty_profile('CZ',
                                                            pulse_profile)
    err_msg = ill_defined_gate_profile(gate_profile)
    assert not err_msg, err_msg

    # test with circuit given
    circuit_config = pulse_profile.pulse_config.circuit_config
    bare_circuit = build_bare_system(ct_params)
    gate_profile: GateProfile = gate.assemble_empty_profile('CZ',
                                                            pulse_profile,
                                                            bare_circuit,
                                                            'foobar')
    err_msg = ill_defined_gate_profile(gate_profile)
    assert not err_msg, err_msg
    assert not (pulse_profile.circuit == gate_profile.circuit),\
        'gate profile not assembled with provided circuit'
    assert gate_profile.circuit_config == 'foobar',\
    ...
    with pytest.warns(UserWarning,
        ...
        gate.assemble_empty_profile(name,
                                    pulse_profile,
                                    bare_circuit)
