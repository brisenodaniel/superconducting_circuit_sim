...
def get_product_comp_states(sys: CompositeSystem,
    ...
    bare_comp_state_tensor = np.empty(
        (comp_states, comp_states, comp_states), dtype=object)
    for n in range(comp_states):
        for m in range(comp_states):
            for k in range(comp_states):
                ...
                product_state = qt.tensor(s0, s1, s2)
                # change basis to dressed basis
                product_state = sys.align_basis(product_state)
                bare_comp_state_tensor[n, m, k] = product_state
                # bare_comp_state_tensor[n, m, k] = qt.tensor(s0, s1, s2)
                # if sys.nlev is not None:
                # bare_comp_state_tensor[n, m, k] = \
                # bare_comp_state_tensor[n, m, k].extract_states(
                # list(range(sys.nlev))
                # )
        ...
        for m in range(comp_states):
            for k in range(comp_states):
                for l, eigenstate in enumerate(eigenstates):
                    overlap = abs(
                        (comp_state_tensor[n, m, k].dag()*eigenstate).tr())
...
def match_states(overlap_tensor: GramTensor,
    ...
    comp_states: CompTensor = np.empty(
        (n_states, n_states, n_states), dtype=object)
    comp_coords: CompCoord = np.empty(
        (n_states, n_states, n_states), dtype=int)
...
def get_bare_comp_states(sys: CompositeSystem,
    ...
    gram_tensor: GramTensor = get_state_overlap(
        product_states, bare_states, comp_states)
...
def get_dressed_comp_states(sys: CompositeSystem,
    ...
    gram_tensor: GramTensor = get_state_overlap(
        bare_comp_states, dressed_eigenstates, comp_states)
...
def get_evolved_comp_state(sys_0: CompositeSystem | CompTensor,
    ...
    overlap_tensor: GramTensor = get_state_overlap(
        comp_states_0, evolved_eigenstates, comp_states)
