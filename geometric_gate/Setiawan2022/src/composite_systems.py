...
class CompositeSystem:
    ...
    def __init__(self, 
      ...
      self.lbl_to_idxs:dict[str:int] = idxs 
    ...
    #   nsys = max(self.lbl_to_idxs.values()) + 1
    #   subsys_dims = np.empty(nsys, dtype=int)
    #   for sys_lbl, idx in self.lbl_to_idxs.items():
    #     subsys_dims[idx] = self.subsystems[sys_lbl].nlev
    ...
    def build_idxs_to_lbl(self)->np.ndarray[str]:
        ...
        for lbl, idx in self.lbl_to_idxs.items():
            idxs_to_lbl[idx] = lbl
        return idxs_to_lbl
    ...
    def plus_interaction_term(self, H_int:Qobj)->CompositeSystem:
        ...
        assert H_int.dims == self.H.dims, \
        f'Operator of dims {self.H.dims} expected, recieved {H_int.dims}'
        new_sys = CompositeSystem(self.subsystems, self.lbl_to_idxs, H_int)
        if self._H_int is not None:
    ...
    def get_raised_op(self, subsys:str, op:str|list[str], op_spec:Callable['...',Qobj]|None=None)->Qobj:
        ...
        Returns:
            Qobj: An operator of the form Id_1 x Id_2 x ... x op_j x ... Id_k, where Id_i is the identity \
                matrix acting on the i'th subsystem in self.lbl_to_idxs, and op_j acts on the j'th subsystem in self.lbl_to_idxs.
        """
        nsys = max(self.lbl_to_idxs.values()) + 1 # number of subsystems
        ...
        subsys_idx = self.lbl_to_idxs[subsys] # index of each subsystem in tensor operation
    ...
    def __eq__(self, other:CompositeSystem)->bool:
       ...
       if self.lbl_to_idxs != other.lbl_to_idxs: return False
